...
export default function Main(props) {
  ...
  const arrPerCheckIn = checkIn.active
  ...
  const arrPerCheckOut =
    checkOut.active && dateCheckOut >= dateCheckIn
      ? arrPerCheckIn.filter((hotel) => {
          return (
  ...
  const arrPerCountry = country
    ? arrPerCheckOut.filter((hotel) => hotel.country === country)
    : arrPerCheckOut;

  //filters the hotels data for the price selection
  const arrPerPrice = price
    ? arrPerCountry.filter((hotel) => hotel.price === price)
    : arrPerCountry;

  //filters the hotels data for the size selection
  const arrPerSize = size
    ? size === "small"
      ? arrPerPrice.filter((hotel) => hotel.rooms <= 10)
      : size === "medium"
      ? arrPerPrice.filter((hotel) => hotel.rooms > 10 && hotel.rooms <= 20)
      : arrPerPrice.filter((hotel) => hotel.rooms > 20)
    : arrPerPrice;
